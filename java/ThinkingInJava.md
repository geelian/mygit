...
# 第15章 泛型 
一般的类和方法，只能使用今天的类型：要么是基本类型，要么是自定义的类。如果要编写可以应用于多种类型的代码，这种刻板的限制对代码的束缚就会很大  
多态算是一种泛化机制  封装 继承 多态  
目的：
1. 要使代码能够应用于“某种不具体的类型”，而不是一个具体的接口或类  
2. 用来指定容器要持有什么类型的对象,而且有编译器来保证类型的正确性  
 
定义：泛型实现了参数化类型的概念，是代码可以应用于多种类型  
术语意思：适用于许多许多的类型  
以前:基础类Object+强转来达到泛型的效果  
核心概念：告诉编译器想使用什么类型，然后编译器帮你处理一切细节  
末端哨兵：来判断堆栈何时为空  

> 业务只有知道了某个技术不能做到什么，你才能更好的做到所能做的  

## 泛型接口&泛型方法
局限性：基本类型无法作为类型参数  
指导原则：无论何时，只要你能做到，你就应该尽量使用泛型方法  
* 对于一个static的方法而言，无法访问泛型类的类型参数，所以,如果static方法需要使用泛型能力，就必须使其成为泛型方法   

|	|泛型类		|泛型方法		|  
|---|-----|----|  
|尽量用	|		|×			|	
|格式	|`class A<T>{}`	|`public<T>void f(T x)`	|  
|指明参数类型|	要	|不要(参数推断)		|  
## 构建复杂模型
使用容器+泛型组成新的数据机构
## 擦除
在泛型代码呢吧，无法获取任何有关泛型参数类型的信息  
java泛型是使用擦除类实现的  

### 泛型擦除的问题
1. 不能显式的运行引用运行是类型的操作之中，如new instanceof
2. 不能继承
...
? 使用类和方法时使用  
T边界   
class A<T extends Has>{} //限定的是 T只能是Has的子类  
类型擦除到第一边界，不是Object而是Has
public <T> void f(T x){}  
?通配符   
List<? extends Fruit> list = new ArrayList<Apple>(); 使用时 只能取不能存  
List<? super Apple> list2 = new ArrayList<Fruit>(); 只能存取只能取出Object   
LIst<?> list = new ArrayList<Fruit>();  
1. 我是想用Java的泛型类编写这段代码，我在这里并不是要用原生类型，但是在当前这种情况下，泛型参数可以持有任何类型
2. 在处理多泛型参数时，有时允许一个参数可以是任何类型，同时为其他参数确定摸着特定了下的这种能力会显得很重要


## 问题
1. 任何基本了下不能作为类型参数， 自动包装不能运行于数组
2. 类型擦除后，接口不能被重复实现
3. 泛型类型参数的转换后instanceof不会有任何效果
4. 泛型参数不参与重载

## 自限定的类型
class SelefBounded<T extends SelfBounded<T>>{}  
循环泛型CRG:我在创建一个新类，它继承自一个泛型类型，这个泛型类型接受我的类的名字作为其参数
CRG本质: 基类用导出类替代其参数  
...
## 异常
类型参数可能会在一个方法的thorws子句中用到，这使你可以编写随检查型异常的类型而发生变化的泛型代码  

## 混型
定义： 混合多个类的能力，以产生一个可以表示混型中所有类型的类  
作用：可以将特性和行为一致的应用于多个类之上  
实现
1. 代理模式 与接口混用
2. 装饰器模式
3. 动态代理
...
## 潜在类型机制
在运行是看他是否有这几个方法  
泛型代码典型的将泛型上调用少量方法，而具有潜在类型机制的语音只要求实现某个方法子集，而不是某个特定类或接口，从而放松了这种限制
...
# 第16章 数组
你可以创建并组装它们，通过使用整型索引值返回它们的元素，并且它们的尺寸不能改变  
数组vs容器：效率、类型、保存基本类型的能力  
_效率--数组是一种效率最高的存储和随机访问对象引用序列的方式_   
类型--可以创建一个数组持有某种具体类型，通过编译期检查，来防止插入错误类型和抽取不当类型  
保存基本类型的能力--数组可以持有基本类型，而泛型之前的容器不行  

对象数组保存的是引用，基本类型数组直接保存基本类型的值  
集聚初始化： int [] i = {1,2,3} // 引用更改，函数参数不能用  
动态集聚初始化： i = new int[]{1,2,3}
## 多维数组
...
System.arraycopy()
> 潜复制
> 不会执行自动包装和自动拆箱，两个数组必须具有相同的确切类型
2. 数组比较
Arrays.equals() Arrays.deepEquals()  
3. 数组元素的比较
> 程序设计的基本目标：将保持不变的事物与发生变化的事物相分离 
3.1 实现java.lang.Comparable接口，使你的类具有"天生"的比较能力 自然排序  
3.2 建立实现Comparator接口的单独的类 定制排序  
4. 数组排序
Arrays.sort
5. 排序好后查找
Arrays.binarySearch()
...
## 填充容器
1. Collections.nCopies & Collectons.fill -》List
2. 使用生成器 适配器
事实上，所有的Collection子类型都有一个接收另一个Collection对象的构造器，用所接收的Collection对象中的元素来填充新的容器。

享元模式：使得对象的一部分可以被具体化，因此，与对象中的所有事物都包含在对象内不同

## Collection 功能方法
add addAll clear contains containAll isEmpty iterator remove removeAll retainAll size toArray 

Arrays.asList() 返回的固定尺寸的List,而Collection.unmodifiableList()产生不可修改的列表  
...
散列的价值在与速度：散列使的查询的以快速进行 

hashCode()
1. 无论何时，对同一对象调用hashCode() 都应该生成同样的值
2. 也不应该使hashCode()依赖于具有唯一性的对象信息，尤其是使用this的值
3. 如果程序中有多个Strin对象，都包括相同的字符串序列，那么这些String对象都映射到同一块内存区域  

HashMap使用的默认负载因子是0.75

# 第18章 Java I/O系统
对于程序语音的设计者来说，创建一个好的输入/输出（I/O）系统是一项艰难的任务
## File类
它既能代表一个特定文件的名称，又能代表一个目录下的一组文件的名称，

## 对象序列化
当你创建对象是，只要你需要，它就会一直存在，但是在程序终止时，无论如何它都不会继续存在
